  ...
  async fetchModels(): Promise<ProcessedModel[]> {
    ...
    try {
      const headers: Record<string, string> = {
        ...
        'User-Agent': 'VSCode-OpenRouter-Copilot-Manager/0.1.0',
      };
      ...
      if (typeof fetch === 'function') {
        Logger.info('Using global fetch for API call...');
        const response = await fetch(MODELS_ENDPOINT, { headers });
        if (!response.ok) {
          const errorText = await response.text();
          throw new Error(`OpenRouter API error ${response.status}: ${errorText}`);
        }
        data = (await response.json()) as OpenRouterModelsResponse;
      } else {
        Logger.info('Using https module fallback for API call...');
        data = await this.httpGetJson<OpenRouterModelsResponse>(MODELS_ENDPOINT, headers);
      }
      ...
      Logger.info(`Fetched ${data.data.length} models from OpenRouter`);
